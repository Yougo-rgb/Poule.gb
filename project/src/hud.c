...
void hud_init(void) {
    //set_sprite_data(35, 8, CustomFont);
    font_init();
    min_font = font_load(font_min);
    font_set(min_font);
    
    unsigned char windowmap[] = {
        0x00, 0x3C, 0x00, 0x1D, 0x1E, 
        0x0B, 0x1C, 0x1E, 0x00, 0x1A, 
        0x19, 0x1F, 0x16, 0x0F, 0x38, 
        0x11, 0x0C, 0x00, 0x00, 0x00
    };
...
void hud_default(void) {
    unsigned char windowmap[] = {
        0x00, 0x00, 0x1A,
        0x19, 0x1F, 0x16, 0x0F, 0x38,
        0x11, 0x0C, 0x00, 0x00, 0x01,
        0x01, 0x39, 0x36, 0x36, 0x3A, 0x00, 0x00,
    };
    set_win_tiles(0, 0, 20, 1, windowmap);
    move_win(7, 0);
}
...
void hud_update_score(uint8_t score, uint8_t goal) {
    ...
    hud_uint_to_hex(goal, 1, goal_hex);
	
    unsigned char windowmap[] = {
        0x00, 0x00, 0x1A, 
        0x19, 0x1F, 0x16, 0x0F, 0x38, 
        0x11, 0x0C, 0x00, 0x00, score_hex[0],
        score_hex[1], 0x39, goal_hex[0], goal_hex[1], 0x3A, 0x00, 0x00,
    };
    set_win_tiles(0, 0, 20, 1, windowmap);
    move_win(7, 0);
}
